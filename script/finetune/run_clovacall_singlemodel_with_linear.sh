...
python -W ignore fairseq_cli/hydra_train.py \
  task.data=$(realpath .)/transcriptions/clovacall/character_spelling \
  checkpoint.save_dir=$(realpath .)/save_checkpoint/finetune/clovacall/single_model_linear \
  task.del_silence=True \
  model.w2v_path=$(realpath .)/save_checkpoint/pretrain/further_pretrain/checkpoint_best.pt \
  --config-dir configs/finetune/single \
  --config-name 100h