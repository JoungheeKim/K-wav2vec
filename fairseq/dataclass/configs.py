...
## 이것도 추가
@dataclass
class AugmentConfig(FairseqDataclass):
    feature_extract_by: str = field(
        default='librosa', metadata={"help": "which library to use for feature extraction"}
    )
    n_mels: int = field(
        default=80, metadata={"help": "Number of mfc coefficients to retain"}
    )
    frame_length: int = field(
        default=20, metadata={"help": "frame length for spectrogram"}
    )
    frame_shift: int = field(
        default=10, metadata={"help": "Length of hop between STFT windows."}
    )
    del_silence: bool = field(
        default=False, metadata={"help": "flag indication whether to delete silence or not"}
    )
    input_reverse: bool = field(
        default=True, metadata={"help": "flag indication whether to reverse input or not"}
    )
    transform_method: str = field(
        default='fbank', metadata={"help": "which feature to use [mel, fbank, spect, mfcc]"}
    )
    freq_mask_para: int = field(
        default=12, metadata={"help": "hyper Parameter for Freq Masking to limit freq masking length"}
    )
    time_mask_num: int = field(
        default=2, metadata={"help": "how many time-masked area to make"}
    )
